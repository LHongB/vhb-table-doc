  ...
  <div class="my-filter-content">
    <div class="my-fc-search">
      <div class="my-fc-search-top">
        <vhb-input v-model="option.data.sVal" placeholder="搜索" suffix-icon="fa fa-search"></vhb-input>
      </div>
        ...
        <template v-if="valList.length">
          <ul class="my-fc-search-list my-fc-search-list-head">
            <li class="my-fc-search-item">
              <vhb-checkbox v-model="isAll" @change="changeAllEvent">全选</vhb-checkbox>
            </li>
          </ul>
          <ul class="my-fc-search-list my-fc-search-list-body">
            <li class="my-fc-search-item" v-for="(item, sIndex) in valList" :key="sIndex">
              <vhb-checkbox v-model="item.checked">{{ item.value }}</vhb-checkbox>
            </li>
          </ul>
        </template>
        <template v-else>
    ...
    <div class="my-fc-footer">
      <vhb-button status="primary" @click="confirmFilterEvent">确认</vhb-button>
      <vhb-button @click="resetFilterEvent">重置</vhb-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    load () {
      ...
      const colValList = Object.keys(HBUtils.groupBy(fullData, column.property)).map(val => {
        return {
