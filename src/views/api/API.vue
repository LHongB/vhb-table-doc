  ...
  <div class="api-view">
    <vhb-grid
      ...
      <template #toolbar_buttons>
        <vhb-input clearable class="search-input" v-model="filterName" type="search" :placeholder="`vhb-${apiName} ${$t('app.api.apiSearch')}`" @keyup="searchEvent" @clear="searchEvent"></vhb-input>
      </template>
...
// const tagMaps = [
//   ['vhb-table', tableAPI, { subtags: ['vhb-colgroup', 'vhb-column'], description: '基础表格' }],
//   ['vhb-colgroup', colgroupAPI, { subtags: ['vhb-column'], description: '基础表格 - 分组列' }],
//   ['vhb-column', columnAPI, { description: '基础表格 - 列' }],
//   ['vhb-grid', gridAPI, { description: '高级表格' }],
//   ['vhb-toolbar', toolbarAPI, { description: '工具栏' }],
//   ['vhb-pager', pagerAPI, { description: '分页' }],
//   ['vhb-radio', radioAPI, { description: '单选框' }],
//   ['vhb-radio-group', radioGroupAPI, { subtags: ['vhb-radio', 'vhb-radio-button'], description: '单选组' }],
//   ['vhb-radio-button', radioButtonAPI, { description: '单选按钮' }],
//   ['vhb-checkbox', checkboxAPI, { description: '复选框' }],
//   ['vhb-checkbox-group', checkboxGroupAPI, { subtags: ['vhb-checkbox'], description: '复选组' }],
//   ['vhb-switch', switchAPI, { description: '开关按钮' }],
//   ['vhb-input', inputAPI, { description: '输入框' }],
//   ['vhb-select', selectAPI, { subtags: ['vhb-optgroup', 'vhb-option'], description: '下拉框' }],
//   ['vhb-optgroup', optgroupAPI, { subtags: ['vhb-option'], description: '下拉框 - 分组' }],
//   ['vhb-option', optionAPI, { description: '下拉框 - 选项' }],
//   ['vhb-button', buttonAPI, { description: '按钮' }],
//   ['vhb-tooltip', tooltipAPI, { description: '工具提示' }],
//   ['vhb-modal', modalAPI, { description: '弹窗' }],
//   ['vhb-form', formAPI, { subtags: ['vhb-form-item'], description: '表单' }],
//   ['vhb-form-item', formItemAPI, { description: '表单 - 项' }],
//   ['vhb-form-gather', formGatherAPI, { description: '表单 - 项集合' }],
//   ['vhb-list', listAPI, { description: '列表' }],
//   ['vhb-pulldown', pulldownAPI, { description: '下拉容器' }]
// ]
...
//   const keys = []
//   props.forEach(item => {
//     const name = HBUtils.kebabCase(item.name)
//     attributes[`${confs[0]}/${name}`] = {
//       type: HBUtils.toValueString(item.type).toLowerCase(),
...
export default {
  ...
  methods: {
    loadList () {
      ...
      return new Promise(resolve => {
        setTimeout(() => {
          ...
          const treeData = HBUtils.clone(apis, true)
          HBUtils.eachTree(treeData, (item, index, items, path, parent) => {
            item.id = primaryKey++
            item.parentId = parent ? parent.id : null
            item.desc = item.descKey ? this.$t(item.descKey) : item.desc
            searchProps.forEach(key => {
              item[key] = HBUtils.escape(item[key])
            })
          }, { children: 'list' })
    ...
    contextMenuClickEvent ({ menu, row, column }) {
      ...
      switch (menu.code) {
        ...
        case 'exportHTMLAPI':
          $grid.exportData({
            type: 'html',
            data: HBUtils.toTreeArray(this.tableData, { children: 'list' }),
            filename: `vhb-${this.apiName}_v${pack.version}`
          })
          break
        case 'exportXLSXAPI':
          $grid.exportData({
        ...
        case 'copy':
          if (row && column) {
            if (XEClipboard.copy(row[column.property])) {
              VHBTable.modal.message({ content: this.$t('app.body.msg.copyToClipboard'), status: 'success' })
            }
          }
          break
        case 'resize':
        ...
        case 'exportAPI':
          $grid.exportData({
            filename: `vhb-${this.apiName}_v${pack.version}.csv`
          })
          break
        case 'allExpand':
    ...
    handleSearch () {
      const filterName = HBUtils.toValueString(this.filterName).trim()
      if (filterName) {
        const options = { children: 'list' }
        if (/pro/i.test(filterName)) {
          const rest = HBUtils.searchTree(this.tableData, item => item.version === 'pro', options)
          this.gridOptions.data = rest
        } else {
          const filterRE = new RegExp(`${filterName}|${HBUtils.camelCase(filterName)}|${HBUtils.kebabCase(filterName)}`, 'i')
          const searchProps = ['name', 'desc', 'type', 'enum', 'defVal', 'version']
          const rest = HBUtils.searchTree(this.tableData, item => searchProps.some(key => filterRE.test(item[key])), options)
