<template>
  <div>
    ...
    <vhb-table
      ...
      <vhb-column field="role" type="html" title="<span class=green>HTML 标签与格式化</span>" :formatter="formatRole"></vhb-column>
    </vhb-table>
        ...
        export default {
          ...
          methods: {
            formatRole ({ cellValue }) {
              return \`<a href="https://github.com/x-extends/vhb-table" class="link" target="_black" style="color: orange">链接 \${cellValue}</a>\`
            },
            filterDescribeMethod ({ value, row, column }) {
              return HBUtils.toValueString(row.html1).indexOf(value) > -1
            }
          }
        }
        `
