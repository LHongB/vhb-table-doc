<template>
  <div>
    ...
    <p class="tip">通过内置事件拦截器可以很容易的处理表格事件行为与第三方组件的兼容性；比如这些插件 <a class="link" href="https://www.npmjs.com/package/vhb-table-plugin-element" target="_blank">vhb-table-plugin-element</a></p>
    <vhb-table
      resizable
      class="api-table"
      ...
      <vhb-column field="version" :title="$t('app.api.title.version')" width="120">
    ...
    <p class="tip">
      ...
      当然还有最简单的用法，只需给弹出面板加上 class=vhb-table--ignore-clear 即可
    </p>
    <pre>
...
export default {
  data () {
    return {
      ...
      demoCodes: [
        `
        VHBTable.interceptor.add('event.clearActived', (params, event) => {
