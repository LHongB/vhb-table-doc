<template>
  <div>
    <p class="tip">
      使用 v-for 去循环静态列是非常糟糕的，仅用于改变属性，需确保 key 唯一性<span class="green">（动态场景需使用 <router-link :to="{name: 'GridBasic'}">vhb-grid</router-link> 进行渲染）</span><br>
    ...
    <vhb-toolbar>
      <template #buttons>
        <vhb-button @click="addColumn()">最后增加一列</vhb-button>
        <vhb-button @click="removeColumn()">删除最后一列</vhb-button>
        <vhb-button @click="updateSexFilter()">修改sex列筛选条件</vhb-button>
        <vhb-button @click="toggleFixedColumn(0, 'left')">切换第一列固定</vhb-button>
        <vhb-button @click="toggleFixedColumn(1, 'left')">切换第二列固定</vhb-button>
        <vhb-button @click="updateWidthColumn(2, 500)">修改第三列宽度</vhb-button>
        <vhb-button @click="updateWidthColumn(3, 500)">修改第四列宽度</vhb-button>
      </template>
    </vhb-toolbar>
