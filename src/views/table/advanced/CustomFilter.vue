<template>
  <div>
    ...
    <vhb-table
      ...
      <vhb-colgroup title="文本">
        <vhb-column field="name" :filters="[{data: ''}]" :filter-method="customStringFilterMethod">
          <template #header="{ column }">
            <vhb-input type="text" v-for="(option, index) in column.filters" :key="index" v-model="option.data" @change="filterEvent(option)" placeholder="请输入" clearable></vhb-input>
          </template>
        </vhb-column>
      </vhb-colgroup>
      <vhb-colgroup title="日期">
        <vhb-column field="date" :filters="[{data: ''}]" :filter-method="customDateFilterMethod">
          <template #header="{ column }">
            <vhb-input type="date" v-for="(option, index) in column.filters" :key="index" v-model="option.data" @change="filterEvent(option)" placeholder="请选择" clearable transfer></vhb-input>
          </template>
        </vhb-column>
      </vhb-colgroup>
      <vhb-colgroup title="下拉框">
        <vhb-column field="sex" :filters="[{data: ''}]" :filter-method="customEqualFilterMethod" :formatter="formatterSex">
          <template #header="{ column }">
            <vhb-select v-for="(option, index) in column.filters" :key="index" v-model="option.data" :options="sexList" @change="filterEvent(option)" placeholder="请选择" clearable transfer></vhb-select>
          </template>
        </vhb-column>
      </vhb-colgroup>
      <vhb-column field="address" title="Address" sortable></vhb-column>
    </vhb-table>
        ...
        export default {
          ...
          methods: {
            ...
            customStringFilterMethod ({ option, row, column }) {
              if (option.data) {
                return HBUtils.toValueString(row[column.property]).toLowerCase().indexOf(option.data) > -1
              }
              return true
            },
            customDateFilterMethod ({ option, row, column }) {
              if (option.data) {
                return HBUtils.isDateSame(row[column.property], option.data, 'yyyy-MM-dd')
              }
              return true
            },
