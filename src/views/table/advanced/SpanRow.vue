<template>
  <div>
    ...
    <vhb-table
      ...
      :data="tableData">
      <vhb-column field="name1" title="功能模块">
        <template #default="{ row }">
          <vhb-checkbox v-model="row.check1" @change="check1ChangeEvent(row, row.check1)">{{ row.name1 }}</vhb-checkbox>
        </template>
      </vhb-column>
      <vhb-column field="name2" title="详细功能">
        <template #default="{ row }">
          <vhb-checkbox v-model="row.check2" @change="check2ChangeEvent(row, row.check2)">{{ row.name2 }}</vhb-checkbox>
        </template>
      </vhb-column>
      <vhb-column field="name3" title="权限类型">
        <template #default="{ row }">
          <vhb-checkbox v-model="row.check3" @change="check3ChangeEvent(row, row.check3)">{{ row.name3 }}</vhb-checkbox>
        </template>
      </vhb-column>
      <vhb-column field="name4" title="权限列表">
        <template #default="{ row }">
          <vhb-checkbox v-model="row.check4" @change="check4ChangeEvent(row, row.check4)">{{ row.name4 }}</vhb-checkbox>
        </template>
      </vhb-column>
    </vhb-table>
    ...
    return {
      ...
      demoCodes: [
        ...
        export default {
          ...
          created () {
            const treeData = HBUtils.toArrayTree(this.getList())
          ...
          methods: {
            ...
            toColTreeData (treeData) {
              ...
              const list = []
              const keyMap = {}
              HBUtils.eachTree(treeData, (item, index, result, paths, parent) => {
                keyMap[item.id] = item
