<template>
  <div>
    ...
    <vhb-table
      ...
      <vhb-column field="name" title="Name" sortable :filters="[{label: 'id大于10', value: 10}, {label: 'id大于40', value: 40}]" :filter-method="filterNameMethod">
      ...
      <vhb-column field="age" title="Age" sortable>
        <template #header="{ column }">
          <span>{{ column.title }}</span>
          <span class="custom-sort" :class="{'is-order': column.order}">
      ...
      <vhb-column field="amount" title="Amount" :formatter="formatAmount" sortable>
        <template #header="{ column }">
          <span>{{ column.title }}</span>
          <span class="custom-sort" :class="{'is-order': column.order}">
        ...
        export default {
          ...
          methods: {
            ...
            formatAmount ({ cellValue }) {
              return HBUtils.commafy(HBUtils.toNumber(cellValue))
            },
