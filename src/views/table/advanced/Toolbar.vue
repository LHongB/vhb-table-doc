<template>
  <div>
    <p class="tip">
      使用自带的工具栏 <toolbar-api-link name="vhb-toolbar"/>，配合模板可以非常简单的实现强大的功能<br>
      支持显示/隐藏列、列宽拖动状态的保存功能，还可以配合 <table-api-link prop="custom"/> 事件实现显示/隐藏列的服务端保存
    </p>

    <vhb-toolbar ref="xToolbar1" custom print></vhb-toolbar>
        ...
        export default {
          ...
          methods: {
            ...
            toolbarCustomEvent (params) {
              ...
              switch (params.type) {
                case 'confirm': {
                  VHBTable.modal.message({ content: \`点击了确认，显示为 \${visibleColumn.length} 列\`, status: 'info' })
                  break
                }
                case 'reset': {
                  VHBTable.modal.message({ content: \`点击了重置，显示为 \${visibleColumn.length} 列\`, status: 'info' })
                  break
                }
                case 'close': {
                  VHBTable.modal.message({ content: \`关闭了面板，显示为 \${visibleColumn.length} 列\`, status: 'info' })
                  break
                }
              }
            }
