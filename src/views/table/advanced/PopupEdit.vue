    ...
    <vhb-table
      ...
      <vhb-column title="操作" width="100" show-overflow>
        <template #default="{ row }">
          ...
          <vhb-button type="text" icon="fa fa-trash-o"></vhb-button>
        </template>
      </vhb-column>
    </vhb-table>

    <vhb-modal v-model="showEdit" :title="selectRow ? '编辑&保存' : '新增&保存'" width="800" min-width="600" min-height="300" resize destroy-on-close>
      <template #default>
        <vhb-form :data="formData" :rules="formRules" title-align="right" title-width="100" @submit="submitEvent">
          ...
          <vhb-form-item field="checkedList" title="可选信息" :span="24" :item-render="{}">
            <template #default="{ data }">
