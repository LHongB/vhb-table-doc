<template>
  <div>
    ...
    <vhb-table
      ...
      <vhb-column field="name" title="Name" :edit-render="{autofocus: '.vhb-input--inner', placeholder: '请点击输入...'}">
        <template #edit="{ row }">
          <vhb-input v-model="row.name" type="text"></vhb-input>
        </template>
      </vhb-column>
      <vhb-column field="nickname" title="Role" :edit-render="{placeholder: '请输入昵称'}">
        <template #edit="{ row }">
          <vhb-input v-model="row.nickname" type="text" placeholder="请输入昵称"></vhb-input>
        </template>
      </vhb-column>
      <vhb-column field="sex" title="Sex" :edit-render="{placeholder: '请选择'}">
      ...
      <vhb-column field="sex2" title="多选下拉" :edit-render="{placeholder: '请选择'}">
        <template #default="{ row }">
          <span>{{ formatMultiSex(row.sex2) }}</span>
        </template>
        <template #edit="{ row }">
          <vhb-select v-model="row.sex2" placeholder="请选择" multiple transfer>
            <vhb-option v-for="item in sexList" :key="item.value" :value="item.value" :label="item.label"></vhb-option>
          </vhb-select>
        </template>
      </vhb-column>
      <vhb-column field="num1" title="Amount" :edit-render="{placeholder: '请输入数值'}">
        <template #edit="{ row }">
          <vhb-input v-model="row.num1" type="float" :digits="2" placeholder="请输入数值"></vhb-input>
        </template>
      </vhb-column>
      <vhb-column field="date12" title="Date" :edit-render="{placeholder: '请选择日期'}">
        <template #edit="{ row }">
