<template>
  <div>
    ...
    <vhb-table
      ...
      <vhb-column field="sex" title="Sex" :edit-render="{autofocus: '.myinput'}">
        <template #edit="scope">
      ...
      <vhb-column field="date3" title="Date" formatter="formatDate" :edit-render="{}">
        <template #edit="scope">
          <input type="date" v-model="scope.row.date3" @input="$refs.xTable.updateStatus(scope)"/>
        </template>
      </vhb-column>
      <vhb-column title="操作" width="200">
        <template #default="{ row }">
          <template v-if="$refs.xTable.isUpdateByRow(row)">
            <vhb-button @click="saveUpdateEvent(row)" :loading="row.loading">局部保存</vhb-button>
          </template>
        </template>
      </vhb-column>
    </vhb-table>
          ...
          methods: {
            ...
            saveUpdateEvent (row) {
              ...
              if ($table.isUpdateByRow(row)) {
                ...
                VHBTable.modal.message({ content: '数据未改动！', status: 'info' })
              }
            },
            saveEvent () {
              this.loading = true
