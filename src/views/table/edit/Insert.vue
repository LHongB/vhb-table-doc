<template>
  <div>
    <p class="tip">
      调用 <table-api-link prop="insert"/>、<table-api-link prop="insertAt"/> 函数插入临时数据，还可以通过 <table-api-link prop="icon"/> 自定义编辑状态的图标，例如第三方图标库：font-awesome、inconfont<br>
      <span class="red">（可以直接使用内置的 CRUD 管理器进行增删改，也可以自行操作数据源，两种方式二选一）</span>
    </p>

    <vhb-toolbar>
      <template #buttons>
        <vhb-button icon="fa fa-plus" @click="insertEvent()">新增</vhb-button>
        <vhb-button @click="insertEvent(tableData[2])">在第3行插入并激活 Sex 单元格</vhb-button>
        <vhb-button @click="insertEvent(-1)">在最后行插入</vhb-button>
        <vhb-button @click="$refs.xTable.removeCheckboxRow()">删除选中</vhb-button>
        <vhb-button @click="getSelectionEvent">获取选中</vhb-button>
        <vhb-button @click="getInsertEvent">获取新增</vhb-button>
        <vhb-button icon="fa fa-save" @click="saveEvent">保存</vhb-button>
      </template>
    </vhb-toolbar>

    <vhb-table
      border
      show-overflow
      keep-source
      ref="xTable"
      max-height="400"
      :data="tableData"
      :edit-config="{trigger: 'click', mode: 'cell', icon: 'fa fa-pencil', showStatus: true}">
      <vhb-column type="checkbox" width="60"></vhb-column>
      <vhb-column type="seq" width="60"></vhb-column>
      <vhb-column field="name" title="Name" sortable :edit-render="{autofocus: '.vhb-input--inner', defaultValue: '默认的名字'}">
        <template #edit="{ row }">
          <vhb-input v-model="row.name" type="text"></vhb-input>
        </template>
      </vhb-column>
      <vhb-column field="sex" title="Sex" :edit-render="{}">
        <template #default="{ row }">
          <span>{{ formatSex(row.sex) }}</span>
        </template>
        <template #edit="{ row }">
          <vhb-select v-model="row.sex" transfer>
            <vhb-option v-for="item in sexList" :key="item.value" :value="item.value" :label="item.label"></vhb-option>
          </vhb-select>
        </template>
      </vhb-column>
      <vhb-column field="age" title="Age" sortable :edit-render="{defaultValue: 18}">
        <template #edit="{ row }">
          <vhb-input v-model="row.age" type="text"></vhb-input>
        </template>
      </vhb-column>
      <vhb-column field="date12" title="Date" sortable :edit-render="{}">
        <template #edit="{ row }">
          <vhb-input v-model="row.date12" type="date" transfer></vhb-input>
        </template>
      </vhb-column>
    </vhb-table>

    <p class="demo-code">{{ $t('app.body.button.showCode') }}</p>

    <pre>
      <pre-code class="xml">{{ demoCodes[0] }}</pre-code>
      <pre-code class="javascript">{{ demoCodes[1] }}</pre-code>
    </pre>
  </div>
</template>

<script>
import VHBTable from 'vhb-table'

export default {
  data () {
    return {
      tableData: [
        { id: 10001, name: 'Test1', nickname: 'T1', role: 'Develop', sex: '0', sex2: ['0'], num1: 40, age: 28, address: 'Shenzhen', date12: '', date13: '' },
        { id: 10002, name: 'Test2', nickname: 'T2', role: 'Designer', sex: '1', sex2: ['0', '1'], num1: 20, age: 22, address: 'Guangzhou', date12: '', date13: '2020-08-20' },
        { id: 10003, name: 'Test3', nickname: 'T3', role: 'Test', sex: '0', sex2: ['1'], num1: 200, age: 32, address: 'Shanghai', date12: '2020-09-10', date13: '' },
        { id: 10004, name: 'Test4', nickname: 'T4', role: 'Designer', sex: '1', sex2: ['1'], num1: 30, age: 23, address: 'Shenzhen', date12: '', date13: '2020-12-04' }
      ],
      sexList: [
        { label: '女', value: '0' },
        { label: '男', value: '1' }
      ],
      demoCodes: [
        `
        <vhb-toolbar>
          <template #buttons>
            <vhb-button icon="fa fa-plus" @click="insertEvent()">新增</vhb-button>
            <vhb-button @click="insertEvent(tableData[2])">在第3行插入并激活 Sex 单元格</vhb-button>
            <vhb-button @click="insertEvent(-1)">在最后行插入</vhb-button>
            <vhb-button @click="$refs.xTable.removeCheckboxRow()">删除选中</vhb-button>
            <vhb-button @click="getSelectionEvent">获取选中</vhb-button>
            <vhb-button @click="getInsertEvent">获取新增</vhb-button>
            <vhb-button icon="fa fa-save" @click="saveEvent">保存</vhb-button>
          </template>
        </vhb-toolbar>

        <vhb-table
          border
          show-overflow
          keep-source
          ref="xTable"
          max-height="400"
          :data="tableData"
          :edit-config="{trigger: 'click', mode: 'cell', icon: 'fa fa-pencil', showStatus: true}">
          <vhb-column type="checkbox" width="60"></vhb-column>
          <vhb-column type="seq" width="60"></vhb-column>
          <vhb-column field="name" title="Name" sortable :edit-render="{autofocus: '.vhb-input--inner', defaultValue: '默认的名字'}">
            <template #edit="{ row }">
              <vhb-input v-model="row.name" type="text"></vhb-input>
            </template>
          </vhb-column>
          <vhb-column field="sex" title="Sex" :edit-render="{}">
            <template #default="{ row }">
              <span>{{ formatSex(row.sex) }}</span>
            </template>
            <template #edit="{ row }">
              <vhb-select v-model="row.sex" transfer>
                <vhb-option v-for="item in sexList" :key="item.value" :value="item.value" :label="item.label"></vhb-option>
              </vhb-select>
            </template>
          </vhb-column>
          <vhb-column field="age" title="Age" sortable :edit-render="{defaultValue: 18}">
            <template #edit="{ row }">
              <vhb-input v-model="row.age" type="text"></vhb-input>
            </template>
          </vhb-column>
          <vhb-column field="date12" title="Date" sortable :edit-render="{}">
            <template #edit="{ row }">
              <vhb-input v-model="row.date12" type="date" transfer></vhb-input>
            </template>
          </vhb-column>
        </vhb-table>
        `,
        `
        import VHBTable from 'vhb-table'
        
        export default {
          data () {
            return {
              tableData: [
                { id: 10001, name: 'Test1', nickname: 'T1', role: 'Develop', sex: '0', sex2: ['0'], num1: 40, age: 28, address: 'Shenzhen', date12: '', date13: '' },
                { id: 10002, name: 'Test2', nickname: 'T2', role: 'Designer', sex: '1', sex2: ['0', '1'], num1: 20, age: 22, address: 'Guangzhou', date12: '', date13: '2020-08-20' },
                { id: 10003, name: 'Test3', nickname: 'T3', role: 'Test', sex: '0', sex2: ['1'], num1: 200, age: 32, address: 'Shanghai', date12: '2020-09-10', date13: '' },
                { id: 10004, name: 'Test4', nickname: 'T4', role: 'Designer', sex: '1', sex2: ['1'], num1: 30, age: 23, address: 'Shenzhen', date12: '', date13: '2020-12-04' }
              ],
              sexList: [
                { label: '女', value: '0' },
                { label: '男', value: '1' }
              ]
            }
          },
          methods: {
            formatSex (value) {
              if (value === '1') {
                return '男'
              }
              if (value === '0') {
                return '女'
              }
              return ''
            },
            async insertEvent (row) {
              const $table = this.$refs.xTable
              const record = {
                sex: '1',
                date12: '2021-01-01'
              }
              const { row: newRow } = await $table.insertAt(record, row)
              await $table.setActiveCell(newRow, 'name')
            },
            getInsertEvent () {
              const $table = this.$refs.xTable
              const insertRecords = $table.getInsertRecords()
              VHBTable.modal.alert(insertRecords.length)
            },
            getSelectionEvent () {
              const $table = this.$refs.xTable
              const selectRecords = $table.getCheckboxRecords()
              VHBTable.modal.alert(selectRecords.length)
            },
            saveEvent () {
              const $table = this.$refs.xTable
              const { insertRecords, removeRecords, updateRecords } = $table.getRecordset()
              VHBTable.modal.alert(\`insertRecords=\${insertRecords.length} removeRecords=\${removeRecords.length} updateRecords=\${updateRecords.length}\`)
            }
          }
        }
        `
      ]
    }
  },
  methods: {
    formatSex (value) {
      if (value === '1') {
        return '男'
      }
      if (value === '0') {
        return '女'
      }
      return ''
    },
    async insertEvent (row) {
      const $table = this.$refs.xTable
      const record = {
        sex: '1',
        date12: '2021-01-01'
      }
      const { row: newRow } = await $table.insertAt(record, row)
      await $table.setActiveCell(newRow, 'name')
    },
    getInsertEvent () {
      const $table = this.$refs.xTable
      const insertRecords = $table.getInsertRecords()
      VHBTable.modal.alert(insertRecords.length)
    },
    getSelectionEvent () {
      const $table = this.$refs.xTable
      const selectRecords = $table.getCheckboxRecords()
      VHBTable.modal.alert(selectRecords.length)
    },
    saveEvent () {
      const $table = this.$refs.xTable
      const { insertRecords, removeRecords, updateRecords } = $table.getRecordset()
      VHBTable.modal.alert(`insertRecords=${insertRecords.length} removeRecords=${removeRecords.length} updateRecords=${updateRecords.length}`)
    }
  }
}
</script>
