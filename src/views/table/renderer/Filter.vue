<template>
  <div>
    <p class="tip">
      筛选渲染 <table-column-api-link prop="filter-render"/>，查看 <a class="link" href="https://github.com/x-extends/vhb-table-docs/tree/main/v4/src/plugins/table/renderer" target="_blank">示例的源码</a><span class="red">（具体请自行实现，该示例仅供参考）</span><br>
      配置参数：<br>
    ...
    <vhb-table
      ...
      <vhb-column field="name" title="文本筛选" :filters="[{data: null}]" :filter-render="{name: 'FilterInput'}"></vhb-column>
      <vhb-column field="role" title="实现条件的筛选" :filters="[{data: {type: 'has', name: ''}}]" :filter-render="{name: 'FilterComplex'}"></vhb-column>
      <vhb-column field="age" title="实现内容的筛选" :filters="[{data: {vals: [], sVal: ''}}]" :filter-render="{name: 'FilterContent'}"></vhb-column>
      <vhb-column field="address" title="实现复杂的筛选" sortable :filters="[{data: {vals: [], sVal: '', fMenu: '', f1Type:'', f1Val: '', fMode: 'and', f2Type: '', f2Val: ''}}]" :filter-render="{name: 'FilterExtend'}"></vhb-column>
    </vhb-table>
  ...
  data  () {
    return {
      ...
      demoCodes: [
        ...
        // 创建一个简单的输入框筛选
        VHBTable.renderer.add('FilterInput', {
          // 筛选模板
