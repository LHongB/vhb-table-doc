<template>
  <div>
    ...
    <p class="tip">
      通过渲染器你可以轻松实现筛选、单元格的复用，可以根据不同业务实现不一样的渲染器，这个功能将非常实用；比如这些插件 <a class="link" href="https://www.npmjs.com/package/vhb-table-plugin-element" target="_blank">vhb-table-plugin-element</a><br>
    ...
    <vhb-table
      ...
      <vhb-column field="name" title="渲染器名称" min-width="100">
        <template #default="{ row }">
          <a class="link" :href="row.url" target="_blank">{{ row.name }}</a>
        </template>
      </vhb-column>
      <vhb-column field="desc" title="说明" min-width="280"></vhb-column>
    </vhb-table>
    <h2>内置渲染器</h2>
    <vhb-table
      resizable
      ...
      <vhb-column field="name" title="渲染器名称" min-width="100">
        <template #default="{ row }">
          <router-link class="link" :to="{name: 'VHBAPI', params: { name: row.compName }}">{{ row.name }}</router-link>
        </template>
      </vhb-column>
      <vhb-column field="desc" title="说明" min-width="280"></vhb-column>
    </vhb-table>
