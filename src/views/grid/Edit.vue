<template>
  <div>
    ...
    <vhb-grid ref="xGrid" v-bind="gridOptions" @page-change="handlePageChange">
      <template #operate="{ row }">
        <template v-if="$refs.xGrid.isActiveByRow(row)">
          <vhb-button icon="fa fa-save" status="primary" title="保存" circle @click="saveRowEvent(row)"></vhb-button>
        </template>
        <template v-else>
          <vhb-button icon="fa fa-edit" title="编辑" circle @click="editRowEvent(row)"></vhb-button>
        </template>
        <vhb-button icon="fa fa-trash" title="删除" circle @click="removeRowEvent(row)"></vhb-button>
        <vhb-button icon="fa fa-eye" title="查看" circle></vhb-button>
        <vhb-button icon="fa fa-gear" title="设置" circle></vhb-button>
      </template>

      <template #name_edit="{ row }">
        <vhb-input v-model="row.name"></vhb-input>
      </template>
      <template #nickname_edit="{ row }">
        <vhb-input v-model="row.nickname"></vhb-input>
      </template>
      <template #sex_default="{ row }">
        <span>{{ formatSex(row.sex) }}</span>
      </template>
      <template #sex_edit="{ row }">
        <vhb-select v-model="row.sex" type="text" transfer>
          <vhb-option v-for="item in sexList1" :key="item.value" :value="item.value" :label="item.label"></vhb-option>
        </vhb-select>
      </template>
      <template #role_edit="{ row }">
        <vhb-input v-model="row.role"></vhb-input>
      </template>
      <template #describe_edit="{ row }">
        <vhb-input v-model="row.describe"></vhb-input>
      </template>
    </vhb-grid>
    ...
    return {
      gridOptions: {
        ...
        columns: [
          ...
          { field: 'name', title: 'Name', editRender: { autofocus: '.vhb-input--inner' }, slots: { edit: 'name_edit' } },
          { field: 'nickname', title: 'Nickname', editRender: { autofocus: '.vhb-input--inner' }, slots: { edit: 'nickname_edit' } },
