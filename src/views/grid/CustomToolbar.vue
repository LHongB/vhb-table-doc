<template>
  <div>
    ...
    <vhb-grid ref="xGrid" v-bind="gridOptions">
      <template #toolbar_buttons>
        <vhb-input v-model="searchName" placeholder="搜索"></vhb-input>
        <vhb-button status="primary" @click="loadData">搜索</vhb-button>
        <vhb-button @click="loadData">刷新</vhb-button>
        <vhb-button @click="insertEvent">新增</vhb-button>
        <vhb-button @click="saveEvent">保存</vhb-button>
        <vhb-button @click="$refs.xGrid.exportData()">导出.csv</vhb-button>
      </template>
      ...
      <template #address_edit="{ row }">
        <vhb-input v-model="row.address"></vhb-input>
      </template>
    </vhb-grid>
          ...
          methods: {
            ...
            saveEvent () {
              setTimeout(() => {
                ...
                VHBTable.modal.message({ content: \`新增 \${insertRecords.length} 条，删除 \${removeRecords.length} 条，更新 \${updateRecords.length} 条\`, status: 'success' })
                this.loadData()
              }, 100)
            }
          }
