<template>
  <div>
    <p class="tip">
      ...
      由 <grid-api-link name="vhb-grid"/> 代理数据转换，只需要配置好数据源即可；非常简单就可以渲染一个表格，从重复写冗余的代码中解放出来
    </p>

    <vhb-grid ref='xGrid' v-bind="gridOptions"></vhb-grid>
      ...
      gridOptions: {
        ...
        formConfig: {
          ...
          items: [
            ...
            { field: 'email', title: '邮件', span: 8, titlePrefix: { useHTML: true, message: '点击链接：<a class="link" href="https://vxetable.cn" target="_blank">vhb-table官网</a>', icon: 'vhb-icon--question' }, itemRender: { name: '$input', props: { placeholder: '请输入邮件' } } },
        ...
        columns: [
          ...
          { field: 'name', title: 'Name', sortable: true, titleHelp: { message: '名称必须填写！' }, editRender: { name: 'input', attrs: { placeholder: '请输入名称' } } },
          {
            ...
            title: 'Role',
            sortable: true,
            titleHelp: { useHTML: true, content: '点击链接：<a class="link" href="https://vxetable.cn" target="_blank">vhb-table官网</a>' },
            filters: [
              { label: '前端开发', value: '前端' },
              { label: '后端开发', value: '后端' },
              { label: '测试', value: '测试' },
        ...
        export default {
          ...
          methods: {
            ...
            formatAmount ({ cellValue }) {
              return cellValue ? \`￥\${HBUtils.commafy(HBUtils.toNumber(cellValue), { digits: 2 })}\` : ''
            },
            ...
            importMethod ({ file }) {
              ...
              return fetch(\`\${this.serveApiUrl}/api/pub/import\`, { method: 'POST', body: formBody }).then(response => response.join()).then(data => {
                const $grid = this.$refs.xGrid
                VHBTable.modal.message({ content: \`成功导入 \${data.result.insertRows} 条记录！\`, status: 'success' })
                // 导入完成，刷新表格
                $grid.commitProxy('query')
              }).catch(() => {
                VHBTable.modal.message({ content: '导入失败，请检查数据是否正确！', status: 'error' })
              })
            },
            // 自定义服务端导出
            exportMethod ({ options }) {
              ...
              return fetch(\`\${this.serveApiUrl}/api/pub/export\`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then(response => response.json()).then(data => {
                if (data.id) {
                  VHBTable.modal.message({ content: '导出成功，开始下载', status: 'success' })
                  // 读取路径，请求文件
                  fetch(\`\${this.serveApiUrl}/api/pub/export/download/\${data.id}\`)
                    .then(response => response.blob())
                    .then(blob => {
                      // 开始下载
                      VHBTable.saveFile({ filename: '导出数据', type: 'xlsx', content: blob })
                    })
                }
              }).catch(() => {
                VHBTable.modal.message({ content: '导出失败！', status: 'error' })
              })
            }
          }
        }
