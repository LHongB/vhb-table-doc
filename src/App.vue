  ...
  <div id="app" @click="clickEvent">
    <header class="page-header">
      <div class="left">
        <a :href="baseApiUrl">
          ...
          <span class="title">vhb-table</span>
        </a>
      </div>
    </header>
    <div class="page-container">
        ...
        <div class="header">
          <!-- <div class="version-list">
            <template v-if="stableVersionList.length">
              ...
              <vhb-select class="stable-select" v-model="selectStableVersion" size="mini" :options="stableVersionList"></vhb-select>
            </template>
            <template v-if="showBetaVetsion">
              <span class="title">{{  $t('app.body.label.latestVersion')}}</span>
              <vhb-select class="latest-select" v-model="selectBetaVersion" size="mini" :options="newBetsVersionList"></vhb-select>
            </template>
          </div> -->
          <vhb-input clearable v-model="filterName" type="search" class="search-input" :placeholder="$t('app.body.search.searchPlaceholder')" @keyup="searchEvent" @clear="searchEvent"></vhb-input>
        </div>
        <div class="body">
              ...
              <ul class="nav-menu">
                <li v-for="(item, index) in apiList" :key="index" :class="{expand: item.expand}">
                  <a class="nav-link" @click="linkEvent(item)" :title="item.disabled ? $t('app.body.other.newFunc') : item.label" :class="{disabled: item.disabled, active: pageKey === item.value}">
                    <i class="vhb-icon--arrow-right nav-link-icon"></i>
      ...
      <div class="oper-wrapper" v-show="showOperBtn">
        <vhb-button class="oper-btn" :icon="showLeft ? 'vhb-icon--arrow-left' : 'vhb-icon--arrow-right'" @click="showLeft = !showLeft"></vhb-button>
      </div>
      <div class="body">
        <div class="content" :class="{full: ['VHBAPI', 'Donation'].includes($route.name)}">
          <div class="page-view">
    ...
    <!-- <vhb-modal v-model="newVersionVisible" width="500" min-height="200" min-width="400" :position="{left: 20, top: 20}" :mask="false" :lock-view="false" show-footer resize>
      <template #title>
      ...
      <template #footer>
        <vhb-button @click="claseNewVersionEvent">不看</vhb-button>
        <vhb-button status="primary" @click="viewNewVersionEvent">查看更多</vhb-button>
      </template>
    </vhb-modal> -->
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    showBetaVetsion () {
      ...
      if (stableVersionList.length) {
        if (betaVersionList.length) {
          ...
          const stable1 = HBUtils.toNumber(stableNums[0])
          const stable2 = HBUtils.toNumber(stableNums[1])
          const stable3 = HBUtils.toNumber(stableNums[2])
          const betaNums = betaVersionList[0].value.split('-')[0].split('.')
          const beta1 = HBUtils.toNumber(betaNums[0])
          const beta2 = HBUtils.toNumber(betaNums[1])
          const beta3 = HBUtils.toNumber(betaNums[2])
          if (beta1 > stable1) {
            return true
    ...
    showOperBtn () {
      return ['StartInstall', 'StartUse', 'StartGlobal', 'StartIcons', 'StartTheme', 'StartI18n', 'VHBAPI', 'Donation'].includes(this.$route.name)
    },
    currentLink () {
      const { $route, tableData } = this
      const childArr = HBUtils.toTreeArray(tableData).filter(data => data.locat && data.locat.name !== 'VHBAPI')
  ...
  created () {
    if (process.env.NODE_ENV === 'development') {
      setInterval(() => {
        ...
        if (performance && performance.memory) {
          this.usedJSHeapSize = HBUtils.toFixed(HBUtils.floor(performance.memory.usedJSHeapSize / 1048576, 2), 2)
        }
      }, 3000)
    } else {
      // setTimeout(() => {
      //   if (location.href.indexOf('vxetable.cn') === -1) {
      //     VHBTable.modal.confirm('官网域名已切换 https://vxetable.cn', '', {
    ...
    // setTimeout(() => {
    //   this.newVersionVisible = localStorage.getItem('VHB_TABLE_NEW_VERSION') !== '4.0'
    // }, 5000)
    this.init()
  },
  methods: {
    ...
    loadList () {
      this.tableData = HBUtils.clone(this.tableList, true)
      HBUtils.eachTree(this.tableData, item => {
    ...
    getVersion () {
      // fetch(`${this.serveApiUrl}/api/npm/versions/vhb-table`)
      //   .then(response => response.json())
      //   .then((data) => {

      const { sp, dp, ss, time, tags } = data
      // console.log('输出的数据', data)
      ...
      // 样式处理
      const serveDate = HBUtils.toStringDate(time)
      const yymmdd = HBUtils.toDateString(serveDate, 'yyyyMMdd')
    ...
    // 调用频率间隔 500 毫秒
    searchEvent: HBUtils.debounce(function () {
      this.handleSearch()
    }, 500, { leading: false, trailing: true }),
    handleSearch () {
      const filterName = HBUtils.toValueString(this.filterName).trim().toLowerCase()
      if (filterName) {
        const filterRE = new RegExp(filterName, 'gi')
        const rest = HBUtils.searchTree(this.tableData, item => item.label.toLowerCase().indexOf(filterName) > -1 || (item.keywords && item.keywords.some((key) => key.toLowerCase().indexOf(filterName) > -1)))
        HBUtils.eachTree(rest, item => {
    ...
    claseNewVersionEvent () {
      ...
      localStorage.setItem('VHB_TABLE_NEW_VERSION', '4.0')
    },
    viewNewVersionEvent () {
      this.claseNewVersionEvent()
      location.href = '/vhb-table/v4/'
    },
    vChangeEvent () {
      switch (this.version) {
        case '1':
