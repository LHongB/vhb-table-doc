...
const toolbarSlots: any = HBUtils.clone(toolbarAPI.find(item => item.name === 'Slots'), true)
...
const pagerSlots: any = HBUtils.clone(pagerAPI.find(item => item.name === 'Slots'), true)
...
const formItemSlots: any = HBUtils.clone(formItemAPI.find(item => item.name === 'Slots'), true)
    ...
    list: [
      {
        ...
        list: HBUtils.clone((formAPI as any[]).find(item => item.name === 'Props'), true).list.map((item: any) => Object.assign({}, item, { name: HBUtils.camelCase(item.name) })).concat([
          {
            name: 'enabled',
            desc: '是否启用',
            version: '3.0.11',
          ...
          },
          {
            ...
            enum: '',
            defVal: '',
            list: HBUtils.clone((formItemAPI as any[]).find(item => item.name === 'Props'), true).list.map((item: any) => Object.assign({}, item, { name: HBUtils.camelCase(item.name) })).concat([
              formItemSlots
            ])
          }
        ])
      ...
      //   list: HBUtils.clone(toolbarAPI.find(item => item.name === 'Props').list, true).concat([
      //     {
      //       name: 'zoom',
      //       desc: '是否允许最大化显示',
      //       version: '',
      ...
      // },
      {
        ...
        enum: '',
        defVal: '继承 setup.grid.toolbarConfig',
        list: HBUtils.clone((toolbarAPI as any[]).find(item => item.name === 'Props').list, true).concat([
          {
            name: 'enabled',
            desc: '是否启用',
            version: '3.0.11',
      ...
      },
      {
        ...
        list: HBUtils.mapTree((pagerAPI as any[]).find(item => item.name === 'Props').list.filter((item: any) => !['size', 'loading'].includes(item.name)), item => Object.assign({}, item, { name: HBUtils.camelCase(item.name) })).concat([
          {
            name: 'enabled',
            desc: '是否启用',
            version: '3.0.11',
...
const gridAPI = HBUtils.clone(tableAPI, true).map(item => {
  const rest: any = apis.find(obj => obj.name === item.name)
  rest.list = (item.list as any[]).concat(rest.list)
  return rest
})

HBUtils.eachTree((gridAPI as any[]), (item, index, obj, paths, parent) => {
...
const columnSlots: any = HBUtils.clone(columnAPI.find(item => item.name === 'Slots'), true)
...
gridAPI.find((item: any) => item.name === 'Props').list.splice(1, 0, {
  ...
  enum: '',
  defVal: '',
  list: HBUtils.mapTree((columnAPI as any[]).find(item => item.name === 'Props').list, item => Object.assign({}, item, { name: HBUtils.camelCase(item.name) })).concat([
    {
      name: 'children',
      desc: '表头分组',
      version: '',
