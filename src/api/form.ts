...
const itemSlots: any = HBUtils.clone(itemAPI.find(item => item.name === 'Slots'), true)
...
const apis = [
  {
    ...
    list: [
      ...
      },
      {
        ...
        enum: '',
        defVal: '',
        list: HBUtils.mapTree(itemProps?.list, (item) => Object.assign({}, item, { name: HBUtils.camelCase(item.name) })).concat([
          {
            name: 'children',
            desc: '项集合',
            version: '3.2.6',
